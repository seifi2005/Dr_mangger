...
/** @var array $errors */
/** @var array $old */
$pageTitle = 'افزودن داروخانه - سیستم مدیریت پزشکان';
?>
...
<div class="main-content" id="mainContent">
    <div class="top-header">
        <div class="d-flex justify-content-between align-items-center">
            <div class="text-start">
                <h1 class="page-title">افزودن داروخانه جدید</h1>
                <p class="breadcrumb-custom">خانه / مدیریت داروخانه / افزودن داروخانه</p>
            </div>
    ...
    <div class="content-area">
        <div class="card-custom">
            <form id="addPharmacyForm" method="POST" action="<?php echo $baseUrl; ?>/pharmacies/store">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label-modal">نام داروخانه <span class="required">*</span></label>
                        <input type="text" class="form-control-modal" name="pharmacyName" id="pharmacyName" placeholder="نام داروخانه را وارد کنید" value="<?php echo htmlspecialchars($old['pharmacyName'] ?? ''); ?>" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control-modal" name="ownerName" id="ownerName" placeholder="نام صاحب را وارد کنید" value="<?php echo htmlspecialchars($old['ownerName'] ?? ''); ?>" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control-modal" name="ownerNationalCode" id="ownerNationalCode" placeholder="کد ملی را وارد کنید" maxlength="10" value="<?php echo htmlspecialchars($old['ownerNationalCode'] ?? ''); ?>" required>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control-modal" name="address" id="address" rows="3" placeholder="آدرس کامل داروخانه را وارد کنید" required><?php echo htmlspecialchars($old['address'] ?? ''); ?></textarea>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-control-modal" name="province" id="province" required onchange="updateCities()">
                            ...
                            <option value="qom" <?php echo ($old['province'] ?? '') === 'qom' ? 'selected' : ''; ?>>قم</option>
                            <option value="tehran" <?php echo ($old['province'] ?? '') === 'tehran' ? 'selected' : ''; ?>>تهران</option>
                            <option value="isfahan" <?php echo ($old['province'] ?? '') === 'isfahan' ? 'selected' : ''; ?>>اصفهان</option>
                            <option value="shiraz" <?php echo ($old['province'] ?? '') === 'shiraz' ? 'selected' : ''; ?>>شیراز</option>
                        </select>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    ...
                    <button type="submit" class="btn-modal-submit">ثبت داروخانه</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function initLocationMap() {
        locationMap = L.map('locationMap').setView([35.6892, 51.3890], 13);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(() => {
            document.getElementById('city').value = '<?php echo $old['city']; ?>';
            updateDistricts();
            <?php if (!empty($old['district'])): ?>
            setTimeout(() => {
                document.getElementById('district').value = '<?php echo $old['district']; ?>';
            }, 100);
            <?php endif; ?>
        }, 100);
        <?php endif; ?>
